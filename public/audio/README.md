...
| El botón de play aparece deshabilitado en Safari | Safari con `preload="metadata"` no dispara `canplay`/`loadeddata` (los eventos que habilitaban el botón) hasta intentar reproducir → dead-end: no puedes pulsar play y sin play no carga | Ya corregido en `AudioPlayer.tsx`: `loadedmetadata` (que Safari sí dispara) habilita el reproductor, y `preload="auto"` fuerza el buffer completo (clips ≤ 450 KB, costo trivial). Regla: nunca depender solo de `canplay`/`loadeddata` para habilitar controles |
| En iPhone el slider de volumen no hace nada | iOS ignora `audio.volume` (lo controla el hardware) | Comportamiento esperado; en móvil el grupo volumen/mute ya está oculto (`sm:`). No intentar "arreglarlo" |
